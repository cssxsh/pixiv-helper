...
object PixivRankCommand : CompositeCommand(
    ...
    suspend fun UserCommandSender.year(year: Year, vararg words: String) = withHelper {
        NaviRank.rank(year = year, words = words).cached(helper = this)
    }

    @SubCommand("month", "月", "月榜")
    @Description("获取NaviRank月榜并随机")
    suspend fun UserCommandSender.month(month: YearMonth, vararg words: String) = withHelper {
        NaviRank.rank(month = month, words = words).cached(helper = this)
    }

    @SubCommand("tag", "标签")
    @Description("获取NaviRank标签榜并随机")
    suspend fun UserCommandSender.tag(vararg words: String) = withHelper {
